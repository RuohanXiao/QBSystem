      ...
      <Row type="flex" justify="space-between" class="code-row-bg" align="middle" :style="{margin:'0'}">
        ...
        <div style="width:80%" id="idNumber2">
          <i class="icon iconfont icon-search" style="position: absolute;
                      ...
                      left:10px;
                      opacity: 0.4;"></i>
          <!-- <input type="text" @change="searchInfo(this.value)"/> -->
          <Select id='ele' v-model="inputInfo" filterable remote placeholder='' :remote-method="searchInfo" @change="queryPerson" :loading="loading1">
                <Option v-for="(option, index) in options1" :value="option.value" :key="index">{{option.label}}</Option>
            </Select>
        </div>
        <div :style="{maxHeight:listHeight,overflowY: 'scroll',textAlign:'left'}">
...
<script>
  ...
  export default {
    ...
    props: ['flag', 'edata'],
    mounted() {
      this.listHeight = document.body.clientHeight * 0.8 - 85 + "px";
